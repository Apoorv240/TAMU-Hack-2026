...
def run_camera(frame_buffer: Queue, buffer: SerialBuffer, joint_buffer: Queue, stop_event: threading.Event):
    cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # try 0, if that fails try 1
    ...
    while not stop_event.is_set():
        ...
        if hands:
            ...
            # Check which fingers are up: [thumb, index, middle, ring, pinky]
            fingers = detector.fingersUp(hand)  # e.g. [0,1,0,0,0] means only index up [web:55][web:58]

            # Simple “index pointing” test: only index finger up
            is_pointing = (fingers == [0, 1, 0, 0, 0])
            if not is_pointing:
                # Skip IK and motor control if not pointing
